...
<body class="bg-maroon-900 min-h-screen relative" style="background-image: url('{{ asset('images/libraryngani.jpg') }}'); background-size: cover; background-position: center; background-attachment: fixed;">
    ...
    <nav class="relative z-20 bg-maroon-950 border-b border-white/10 px-6 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-6">
            <a href="{{ url('/librarian/dashboard') }}" class="text-gray-300 hover:text-white text-sm {{ request()->is('librarian/dashboard') ? 'text-white font-medium' : '' }}">Dashboard</a>
            <a href="{{ url('/librarian/books') }}" class="text-gray-300 hover:text-white text-sm {{ request()->is('librarian/books*') ? 'text-white font-medium' : '' }}">Books</a>
            <a href="{{ url('/librarian/borrows') }}" class="text-gray-300 hover:text-white text-sm {{ request()->is('librarian/borrows*') ? 'text-white font-medium' : '' }}">Borrows</a>
