...
<div class="bg-library-card rounded-2xl p-6 border border-white/5"><p class="text-gray-400 text-sm">Overdue</p><h3 class="text-3xl font-bold text-red-300 mt-2">{{ number_format($stats['overdueCount'] ?? 0) }}</h3></div>
<div class="bg-library-card rounded-2xl p-6 border border-white/5"><p class="text-gray-400 text-sm">Unpaid Fines</p><h3 class="text-3xl font-bold text-amber-300 mt-2">${{ number_format($stats['unpaidFines'] ?? 0, 2) }}</h3></div>
</div>
...
@php($fine = $isOverdue ? $br->calculateFine() : 0)
<div class="flex items-center justify-between py-2 border-b border-white/5">
    <div>
        <p class="text-white text-sm">{{ $br->user?->name ?? 'Someone' }} {{ $isReturned ? 'returned' : 'borrowed' }} "{{ $br->book?->title ?? 'a book' }}"</p>
        <p class="text-gray-500 text-xs">{{ $br->borrowed_at?->diffForHumans() }} @if ($fine > 0) · <span class="text-amber-300">Fine: ${{ number_format($fine, 2) }}</span> @endif</p>
    </div>
